...
const ConnectWalletMenu: React.FC<ConnectWalletMenuProps> = ({
  ...
  const [hasCopiedToClipboard, setHasCopiedToClipboard] =
  ...
  const [isInjectedProviderAuthorized, setIsInjectedProviderAuthorized] =
    useState<boolean>(false);
  useEffect(() => {
  ...
  const handleWeb3Activation = useCallback(
    async (connector: AvailableConnectors) => {
      ...
      let walletName: AvailableConnectorKeys | InjectedProviders | undefined =
        connectorName;
